...
module sdram_controller_2(
    ...
    // NOTE: For MisTer SDRAM module, LDQM is shorted to A[11] and UDQM is shorted to A[12]
...
logic        load_mode;
logic        freeze;
logic        burst_active;  // Indicates A[12:11] are being used for DQM
logic        sdram_idle;
...
// NOTE: LDQM/UDQM are incorporated into DRAM_ADDR[12:11] for MisTer module
...
always_comb begin
    ...
    sdram_ack = 1'b0;
    next_burst_counter = (burst_counter > 0) ? burst_counter - 1'b1 : 0;
    
    // During a burst, A[12:11] are used for DQM and must not be changed
    burst_active = (burst_counter > 0);
    ...
    if (bank_read != 4'b0000 && burst_counter==4'h0) begin
        cmd  = CMD_READ;
        // For READ: set DQM bits (A[12:11]) to 00 to enable both byte lanes
        addr = {1'b0, 2'b00, sdram_addr[10:2], 1'b0};
        ...
        cmd  = CMD_WRITE;
        // For WRITE: set DQM bits (A[12]=UDQM, A[11]=LDQM) from write_strobe
        // write_strobe will contain inverted byte enables
        addr = {1'b0, ~sdram_wstrb[1], ~sdram_wstrb[0], sdram_addr[10:2], 1'b0};
    ...
    end else if (bank_activate != 4'b0000 && !burst_active) begin
        // Only ACTIVATE when no burst is active (A[12:11] must not change during bursts)
        cmd  = CMD_ACTIVATE;
